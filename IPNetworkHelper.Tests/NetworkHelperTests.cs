...
public class NetworkHelperTests
{
    ...
    public void TryParseInvalidCIDRIPv6()
        => Assert.IsFalse(IPNetwork.TryParse("::1/1/2", out var _));

    [TestMethod]
    public void TryParseCIDRIPv4()
    {
        Assert.IsTrue(IPNetwork.TryParse("192.168.0.0/24", out var network));
        Assert.AreEqual(24, network.PrefixLength);
        Assert.IsTrue(IPAddress.Parse("192.168.0.0").Equals(network.BaseAddress));
    }

    [TestMethod]
    public void TryParseCIDRIPv6()
    {
        Assert.IsTrue(IPNetwork.TryParse("DEAD:BEEF:0:1234::/64", out var network));
        Assert.AreEqual(64, network.PrefixLength);
        Assert.IsTrue(IPAddress.Parse("DEAD:BEEF:0:1234::").Equals(network.BaseAddress));
    }

    [TestMethod]
    public void TryParsePrefixMustBeNetworkIPv4()
    {
        Assert.IsFalse(IPNetwork.TryParse("192.168.0.150/24", out var _));
        Assert.IsTrue(IPNetwork.TryParse("192.168.0.0/24", out var _));
    }

    [TestMethod]
    public void TryParsePrefixMustBeNetworkIPv6()
    {
        Assert.IsFalse(IPNetwork.TryParse("DEAD:BEEF:0:1234::FF00/64", out var _));
        Assert.IsTrue(IPNetwork.TryParse("DEAD:BEEF:0:1234::/64", out var _));
    }

    [TestMethod]
    public void TryParsePrefixLengthMustBeValid()
    {
        Assert.IsFalse(IPNetwork.TryParse("192.168.0.0/33", out var _));            // Should fail, max is 32
        Assert.IsFalse(IPNetwork.TryParse("0.0.0.0/-1", out var _));                // Should fail, min is 0

        Assert.IsFalse(IPNetwork.TryParse("DEAD:BEEF:0:1234::/129", out var _));    // Should fail, max is 128
        Assert.IsFalse(IPNetwork.TryParse("0::0/-1", out var _));                   // Should fail, min is 0

        // These should all pass since they're at the limits (min/max)
        Assert.IsTrue(IPNetwork.TryParse("192.168.0.0/32", out var _));
        Assert.IsTrue(IPNetwork.TryParse("0.0.0.0/0", out var _));

        Assert.IsTrue(IPNetwork.TryParse("DEAD:BEEF:0:0:1234:5678:90AB:CDEF/128", out var _));
        Assert.IsTrue(IPNetwork.TryParse("0::0/0", out var _));
    }

    [TestMethod]
    public void TryParseHandlesNullEmptyAndInvalidValues()
    {
        Assert.IsFalse(IPNetwork.TryParse(null!, out var _));
        Assert.IsFalse(IPNetwork.TryParse(string.Empty, out var _));
        Assert.IsFalse(IPNetwork.TryParse("invalid", out var _));
    }

    [TestMethod]
    public void SplitIPv4()
    {
        var network = IPNetwork.Parse("192.168.0.0/24");
        var (left, right) = network.Split();

        Assert.IsTrue(IPAddress.Parse("192.168.0.0").Equals(left.BaseAddress));
        Assert.AreEqual(25, left.PrefixLength);
        Assert.IsTrue(IPAddress.Parse("192.168.0.128").Equals(right.BaseAddress));
    ...
    {
        var network = IPNetwork.Parse("DEAD:BEEF:0:1234::/64");
        var (left, right) = network.Split();

        Assert.IsTrue(IPAddress.Parse("DEAD:BEEF:0:1234::0").Equals(left.BaseAddress));
        Assert.AreEqual(65, left.PrefixLength);
        Assert.IsTrue(IPAddress.Parse("DEAD:BEEF:0:1234:8000::0").Equals(right.BaseAddress));
    ...
    {
        var network = IPNetwork.Parse("192.168.0.0/32");
        network.Split();
    }
    ...
    {
        var network = IPNetwork.Parse("DEAD:BEEF:0:0:1234:5678:90AB:CDEF/128");
        network.Split();
    }
    ...
    public void ExtractThrowsOnAddressFamilyMismatch()
    {
        ...
        var ipv6 = IPNetwork.Parse("DEAD:BEEF:0:1234::/64");
    ...
    {
        var network = IPNetwork.Parse("192.168.0.0/24");
        var biggernetwork = IPNetwork.Parse("192.168.0.0/23");
    ...
    {
        var network = IPNetwork.Parse("192.168.0.0/24");
        var different = IPNetwork.Parse("172.16.0.0/30");
    ...
    public void ExtractIPv4()
    {
        ...
        var desired = IPNetwork.Parse("192.168.16.16/28");

        var result = network.Extract(desired).ToArray();
        var expected = new[] { "192.168.0.0/20", "192.168.16.0/28", "192.168.16.16/28", "192.168.16.32/27", "192.168.16.64/26", "192.168.16.128/25", "192.168.17.0/24", "192.168.18.0/23", "192.168.20.0/22", "192.168.24.0/21", "192.168.32.0/19", "192.168.64.0/18", "192.168.128.0/17" }.Select(IPNetwork.Parse).ToArray();
    ...
    {
        var network = IPNetwork.Parse("192.168.0.0/16");
        var desired = IPNetwork.Parse("192.168.0.0/16");
    ...
    public void ExtractIPv6()
    {
        ...
        var desired = IPNetwork.Parse("1111:2222:3333:8840::/64");

        var result = network.Extract(desired).ToArray();
        var expected = new[] { "1111:2222::/35", "1111:2222:2000::/36", "1111:2222:3000::/39", "1111:2222:3200::/40", "1111:2222:3300::/43", "1111:2222:3320::/44", "1111:2222:3330::/47", "1111:2222:3332::/48", "1111:2222:3333::/49", "1111:2222:3333:8000::/53", "1111:2222:3333:8800::/58", "1111:2222:3333:8840::/64", "1111:2222:3333:8841::/64", "1111:2222:3333:8842::/63", "1111:2222:3333:8844::/62", "1111:2222:3333:8848::/61", "1111:2222:3333:8850::/60", "1111:2222:3333:8860::/59", "1111:2222:3333:8880::/57", "1111:2222:3333:8900::/56", "1111:2222:3333:8a00::/55", "1111:2222:3333:8c00::/54", "1111:2222:3333:9000::/52", "1111:2222:3333:a000::/51", "1111:2222:3333:c000::/50", "1111:2222:3334::/46", "1111:2222:3338::/45", "1111:2222:3340::/42", "1111:2222:3380::/41", "1111:2222:3400::/38", "1111:2222:3800::/37", "1111:2222:4000::/34", "1111:2222:8000::/33" }.Select(IPNetwork.Parse).ToArray();
    ...
    {
        var network = IPNetwork.Parse("1111:2222::/32");
        var desired = IPNetwork.Parse("1111:2222::/32");

        var result = network.Extract(desired).ToArray();
        var expected = new[] { IPNetwork.Parse("1111:2222::/32") };
    ...
    {
        var network = IPNetwork.Parse("192.168.0.0/16");
        var desired = new[] { "192.168.239.252/30", "192.168.228.0/24", "192.168.174.0/24" }.Select(IPNetwork.Parse).ToArray();

        var result = network.Extract(desired).ToArray();
        var expected = new[] { "192.168.0.0/17", "192.168.128.0/19", "192.168.160.0/21", "192.168.168.0/22", "192.168.172.0/23", "192.168.174.0/24", "192.168.175.0/24", "192.168.176.0/20", "192.168.192.0/19", "192.168.224.0/22", "192.168.228.0/24", "192.168.229.0/24", "192.168.230.0/23", "192.168.232.0/22", "192.168.236.0/23", "192.168.238.0/24", "192.168.239.0/25", "192.168.239.128/26", "192.168.239.192/27", "192.168.239.224/28", "192.168.239.240/29", "192.168.239.248/30", "192.168.239.252/30", "192.168.240.0/20" }.Select(IPNetwork.Parse).ToArray();
    ...
    public void ExtractMultipleSkipsCreated()
    {
        ...
        var network = IPNetwork.Parse("37.0.0.0/8");
        var desired = new[] { "37.10.128.0/17", "37.12.128.0/18", "37.13.64.0/18", "37.13.128.0/17" }.Select(IPNetwork.Parse).ToArray();

        var result = network.Extract(desired).ToArray();
        var expected = new[] { "37.0.0.0/13", "37.8.0.0/15", "37.10.0.0/17", "37.10.128.0/17", "37.11.0.0/16", "37.12.0.0/17", "37.12.128.0/18", "37.12.192.0/18", "37.13.0.0/18", "37.13.64.0/18", "37.13.128.0/17", "37.14.0.0/15", "37.16.0.0/12", "37.32.0.0/11", "37.64.0.0/10", "37.128.0.0/9" }.Select(IPNetwork.Parse).ToArray();
    ...
    {
        var network = IPNetwork.Parse("192.168.0.0/16");
        var desired = new[] { "192.168.239.252/30", "192.168.228.0/24", "10.10.5.0/24" }.Select(IPNetwork.Parse).ToArray();
